				...
				u16 addr, u16 val)
{
	...
	LOG_INFO("addr: %04x val:%02x",addr, e);
...
getMAX9295GPIO(
    uint8_t gpioInd)
{
    GPIOTypeMAX9295 gpio = MAX9295_GPIO_TYPE_INVALID;

    switch (gpioInd) {
        case 0:
            gpio = MAX9295_GPIO_TYPE_MFP0;
            break;
        case 1:
            gpio = MAX9295_GPIO_TYPE_MFP1;
            break;
        case 2:
            gpio = MAX9295_GPIO_TYPE_MFP2;
            break;
        case 3:
            gpio = MAX9295_GPIO_TYPE_MFP3;
            break;
        case 4:
            gpio = MAX9295_GPIO_TYPE_MFP4;
            break;
        case 5:
            gpio = MAX9295_GPIO_TYPE_MFP5;
            break;
        case 6:
            gpio = MAX9295_GPIO_TYPE_MFP6;
            break;
        case 7:
            gpio = MAX9295_GPIO_TYPE_MFP7;
            break;
        case 8:
            gpio = MAX9295_GPIO_TYPE_MFP8;
            break;
        default:
            LOG_ERROR("MAX96712: Invalid Max9295 GPIO pin %d", (uint32_t)gpioInd);
    }

    return gpio;
}

static NvMediaStatus SetupAddressTranslations(struct ar0233 *priv)
{
	NvMediaStatus nvmStatus = WICRI_STATUS_OK;
    ReadWriteParamsMAX9295 paramsMAX9295 = {};
	max9295 *ser = priv->priv_ser;

    // Check serializer is present
    LOG_INFO("Check broadcast serializer is present\n");
    nvmStatus = MAX9295CheckPresence(priv->priv_prim_ser);
    if (nvmStatus != WICRI_STATUS_OK) {
        LOG_ERROR("MAX9295: MAX9295CheckPresence(ser) failed with NvMedia error");
        return nvmStatus;
    }

    // Set unique address with broadcast address
    LOG_INFO("Set unique address!!!!!!!!!!!!!!!\n");
    paramsMAX9295.DeviceAddress.address = ser->i2c_client->addr;
	LOG_INFO("priv->priv_prim_ser->i2c_client->addr : 0x%02x", priv->priv_prim_ser->i2c_client->addr);
    nvmStatus = MAX9295WriteParameters(priv->priv_prim_ser,
                                       WRITE_PARAM_CMD_MAX9295_SET_DEVICE_ADDRESS,
                                       sizeof(paramsMAX9295.DeviceAddress.address),
                                       &paramsMAX9295);
    if (nvmStatus != WICRI_STATUS_OK) {
        LOG_ERROR("MAX9295: WRITE_PARAM_CMD_MAX96705_SET_DEVICE_ADDRESS failed with NvMedia error");
        return nvmStatus;
    }
...
NvMediaStatus transportLink_Max96712_9295(struct ar0233 *priv)
{
	max96712* dser = priv->priv_der;
	max9295*  ser  = priv->priv_ser;
	NvMediaStatus nvmStatus=WICRI_STATUS_OK;
	ReadWriteParamsMAX9295 paramsMAX9295 = {};
	...
	nvmStatus=EnableLinks(priv->priv_der,GetMAX96712Link(priv->link_index));
	if (nvmStatus!=WICRI_STATUS_OK)
	{
		LOG_ERROR("MAX96712:EnableLinks Failed!");
		return nvmStatus;
	}
	msleep(100);
    ...
    nvmStatus = SetupAddressTranslations(priv);
    if (nvmStatus != WICRI_STATUS_OK) {
        LOG_ERROR("MAX96712: SetupAddressTranslations failed with SIPL error %d", (int32_t)nvmStatus);
        return nvmStatus;
    }


    LOG_INFO("WRITE_PARAM_CMD_MAX9295_ENABLE_NVP2650!!!!!!!!!!!!!!!!!!\n");
    nvmStatus = MAX9295WriteParameters(ser,
                                       WRITE_PARAM_CMD_MAX9295_ENABLE_NVP2650,
                                       sizeof(paramsMAX9295.ConfigPhy),
                                       &paramsMAX9295);
    if (nvmStatus != WICRI_STATUS_OK) {
        LOG_ERROR("MAX96712: WRITE_PARAM_CMD_MAX9295_CONFIG_PHY failed with NvMedia error");
        return nvmStatus;
    }
	msleep(50);
	
	//lower reset GPIO, wait start streaming to output image data.
	paramsMAX9295.GPIOOutp.gpioInd = getMAX9295GPIO(0);
	paramsMAX9295.GPIOOutp.level = false;
	nvmStatus = MAX9295WriteParameters(priv->priv_ser,
										WRITE_PARAM_CMD_MAX9295_SET_GPIO_OUTPUT,
										sizeof(paramsMAX9295.GPIOOutp),
										&paramsMAX9295);
	if (nvmStatus != WICRI_STATUS_OK) {
		LOG_ERROR("MAX96712: WRITE_PARAM_CMD_MAX9295_SET_GPIO_OUTPUT failed with NvMedia error");
		return nvmStatus;
	}
	msleep(50);

    pipeLineMap = &paramsMAX96712.PipelineMapping.linkPipelineMap[priv->link_index];
    pipeLineMap->isEmbDataType = false;
    pipeLineMap->vcID = priv->g_ctx.dst_vc;
    pipeLineMap->dataType = MAX96712_DATA_TYPE_YUV_8;
    pipeLineMap->isDTOverride = false;
    paramsMAX96712.PipelineMapping.link = GetMAX96712Link(priv->link_index);
    nvmStatus = MAX96712WriteParameters(dser,
                                        WRITE_PARAM_CMD_MAX96712_PIPELINE_MAPPING_YUV,
                                        sizeof(paramsMAX96712.PipelineMapping),
                                        &paramsMAX96712);
    if (nvmStatus != WICRI_STATUS_OK) {
        LOG_ERROR("MAX96712: WRITE_PARAM_CMD_MAX96712_PIPELINE_MAPPING_YUV failed with NvMedia error");
        return nvmStatus;
    }
...
int ar0233_gmsl_serdes_setup(struct ar0233 *priv)
{
	int err = 0;
	struct device *dev;
...
static int ar0233_set_mode(struct tegracam_device *tc_dev)
{
	...
	LOG_INFO("mode:%d",s_data->mode_prop_idx);
...
static int ar0233_start_streaming(struct tegracam_device *tc_dev)
{
	...
	paramsMAX9295.GPIOOutp.gpioInd = getMAX9295GPIO(0);
	paramsMAX9295.GPIOOutp.level = true;
	nvmStatus = MAX9295WriteParameters(priv->priv_ser,
										WRITE_PARAM_CMD_MAX9295_SET_GPIO_OUTPUT,
										sizeof(paramsMAX9295.GPIOOutp),
										&paramsMAX9295);
	...
	printk(KERN_ERR "alex: %s ar0233_start_streaming\n", tc_dev->name);
...
static int ar0233_board_setup(struct ar0233 *priv)
{
	...
	priv->priv_der = dev_get_drvdata(priv->dser_dev);
	...
	err = of_property_read_u32(gmsl, "channel-link", &value);
	if (err < 0) {
		dev_err(dev, "No channel-link info\n");
		goto error;
	}
	priv->link_index = value;
			...
			const struct i2c_device_id *id)
{
	...
	printk("alex:transportLink_Max96712_9295\n");
	err = transportLink_Max96712_9295(priv);
	if (err) {
		dev_err(&client->dev,
			"%s gmsl transportLink_Max96712_9295 failed\n", __func__);
		return err;
	}
	LOG_INFO("Enabling links\n");
	writeParamsMAX96712.link = priv->priv_der->ctx.linkMask;
	err = MAX96712WriteParameters(priv->priv_der,
									WRITE_PARAM_CMD_MAX96712_ENABLE_SPECIFIC_LINKS,
									sizeof(writeParamsMAX96712.link),
									&writeParamsMAX96712);
	if (err) {
		dev_err(dev, "enable all links failed\n");
		return err;
	}
