#ifndef _MAX96712_PRIV_H_
#define _MAX96712_PRIV_H_
 ...
 * The macro expects the following variables to be available: priv
 */
#define GET_FIELD_FROM_QUEUE_RET_ERR(index, val)                                 \
                                {                                               \
                                    status = ReadFromRegFieldQ(priv,          \
                                                               index,           \
                                                               val);            \
                                    if (status != WICRI_STATUS_OK) {          \
                                        return status;                          \
...
#define UPDATE_GLOBAL_ERROR(error, last) \
    {                                                                              \
        ...
        if (errorStatus->count < MAX96712_MAX_ERROR_STATUS_COUNT) {                \
            ...
            LOG_ERROR("MAX96712: error count over max");                             \
        }                                                                          \
...
#define MAX96712_DEVICE_INDEX                         0u
...
void max96712_power_off(
    struct device *dev);

#endif /* _MAX96712_PRIV_H_ */